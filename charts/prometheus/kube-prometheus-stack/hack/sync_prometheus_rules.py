...
condition_map = {
    ...
    # kubernetes-system was split into more groups in 1.14, one of them is kubernetes-system-apiserver
    'kubernetes-system-apiserver': ' .Values.defaultRules.rules.kubernetesSystem',
    # kubernetes-system was split into more groups in 1.14, one of them is kubernetes-system-kubelet
    ...
    # kube-prometheus >= 1.14 uses prometheus as group instead of prometheus.rules
...
alert_condition_map = {
    ...
    # there are more alerts which are left enabled, because they'll never fire without metrics
...
def yaml_str_repr(struct, indent=4):
    ...
    text = textwrap.indent(text, ' ' * indent)[indent - 1:]
    return text
        ...
        if line_start + alert_name in rules:
            ...
            while True:
                try:
                    ...
                    # add end of if
                    try:
                        next_index = rules.index(
                            line_start, index + len(rule_text) + 1)
                    except ValueError:
                    ...
                    while not found_block_end:
                        ...
                        if last_line.startswith('{{- if'):
                            ...
                            next_index = last_line_index + 1
                            continue

                        found_block_end = True
                    rules = rules[:next_index] + \
                        '{{- end }}\n' + rules[next_index:]
                except ValueError:
                    break
    return rules
...
def add_rules_per_rule_conditions(rules, group, indent=4):
    ...
    for rule in group['rules']:
        if 'alert' in rule:
            rules_condition_map[rule['alert']
...
{{- end }}""" % (rule_group_labels,), ' ' * (indent + label_indent * 2))
    ...
    ruleStartingLine = [(rule_position.start(), rule_position.end())
    ...
    for i, rule in enumerate(rules):
        ...
        if current_label:
            ...
            if entries:
                ...
                rules[i] = rule[:entries_end] + additional_rule_labels_condition_start + \
                    additional_rule_labels + \
                    additional_rule_labels_condition_end + rule[entries_end:]
            else:
                # `labels:` does not contain any entries
                # append template to label section
                rules[i] += additional_rule_labels_condition_start + \
                    additional_rule_labels + additional_rule_labels_condition_end
        else:
            # `labels:` block does not exist
            # create it and append template
            rules[i] += additional_rule_labels_condition_start + '\n' + ' ' * indent + \
                '  labels:' + additional_rule_labels + additional_rule_labels_condition_end
...
def add_custom_annotations(rules, group, indent=4):
    ...
    rule_group_labels = get_rule_group_condition(condition_map.get(
        group['name'], ''), 'additionalRuleGroupAnnotations')
    ...
    for alert_position in alerts_positions:
        ...
        index = alert_position.end() + annotations_len + (rule_condition_len +
                                                          rule_group_condition_len) * alert
        rules = rules[:index] + '\n' + rule_condition + \
            rule_group_condition + rules[index:]
        alert += 1
...
def main():
    ...
    for chart in charts:
        if 'git' in chart:
            ...
            subprocess.run(['git', 'init', '--initial-branch',
                           'main', checkout_dir, '--quiet'])
            subprocess.run(['git', '-C', checkout_dir, 'remote',
                           'add', 'origin', chart['git']])
            subprocess.run(['git', '-C', checkout_dir, 'fetch',
                           '--depth', '1', 'origin', branch, '--quiet'])
            subprocess.run(['git', '-c', 'advice.detachedHead=false',
                           '-C', checkout_dir, 'checkout', 'FETCH_HEAD', '--quiet'])
            ...
            if chart.get('mixin'):
                ...
                alerts = json.loads(_jsonnet.evaluate_snippet(
                    mixin_file, chart['mixin'], import_callback=jsonnet_import_callback))
        ...
        groups = alerts['spec']['groups'] if alerts.get(
            'spec') else alerts['groups']
        for group in groups:
            write_group_to_file(
                group, url, chart['destination'], chart['min_kubernetes'], chart['max_kubernetes'])
